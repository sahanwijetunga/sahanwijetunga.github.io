...
I implemented definitions for Hyperbolic Spaces, namely vector spaces \\(V\\) equipped with a bilinear form \\(B\\) that is a direct sum of Hyperbolic 2-spaces, those with a basis \\((e,f)\\) with \\(B(e,e)=B(f,f)=0\\) and \\(B(e,f)=1\\) (\\(B(f,e)\\) is left unspecified, however if the form is alternating or reflexive the value is forced). 
